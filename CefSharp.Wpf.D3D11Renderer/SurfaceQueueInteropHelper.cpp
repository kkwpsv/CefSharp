...
namespace Microsoft {
    namespace Windows {
        namespace Media {
            ...
            void SurfaceQueueInteropHelper::QueueHelper(QueueRenderMode renderMode, Int32Rect dirtyRect)
            {
                HRESULT hr = S_OK;
            ...
            Cleanup:
                if (fNeedUnlock)
                {
                    if (dirtyRect.X + dirtyRect.Width <= m_d3dImage->PixelWidth && dirtyRect.Y + dirtyRect.Height <= m_d3dImage->PixelHeight)
                    {
                        m_d3dImage->AddDirtyRect(dirtyRect);
                    }
                    else
                    {
            ...
            void SurfaceQueueInteropHelper::SetPixelSize(unsigned int pixelWidth, unsigned int pixelHeight)
            {
                    ...
                    (m_pixelHeight != pixelHeight))
                {
                    ...
                    m_pixelHeight = pixelHeight;
                    CleanupSurfaces();
                    QueueHelper(QueueRenderMode::RenderDXGI, Int32Rect(0, 0, m_d3dImage->PixelWidth, m_d3dImage->PixelHeight));
                }
            }

            void SurfaceQueueInteropHelper::RequestRenderD2D(Int32Rect dirtyRect)
            {
                QueueHelper(QueueRenderMode::RenderDXGI, dirtyRect);
            }
